...
use App\Http\Picl0u\Payment\CitelisPayment;
use App\Http\Picl0u\Payment\PaypalPayment;
...
use App\Mail\ProductQuantityAlert;
...
use Modules\ShoppingCart\Http\Requests\ExpressUsers;
use Modules\ShoppingCart\Http\Requests\SelectAddresses;
use Modules\ShoppingCart\Http\Requests\SelectCarrier;
...
class ShoppingCartController extends Controller
{
    ...
    protected $viewPath = "shoppingcart::";
    ...
    public function addProduct(Request $request)
    {
        ...
        if(isset($request->declinaisons) && !empty($request->declinaisons)) {
            ...
            $attribute = ProductsAttribute::where('product_id', $product->id)
                ->where(function($query) use ($declinaisons) {
                    foreach($declinaisons as $key => $value) {
                        $query->where('declinaisons->'.$key, $value);
                    }
            })->first();
            ...
            if(!empty($attribute->reference) && !is_null($attribute->reference)) {
                $product->reference = $attribute->reference;
            }
            $product->stock_available = $attribute->stock_brut;
        ...
        $cartitem->associate('Modules\Product\Http\Product');
        ...
        $cart = [
            ...
            'total' => "<strong>Total : </strong>" . priceFormat($total),
            'shipping' => "<strong>Frais de port : </strong>" . priceFormat($shippingPrice),
            'vat' => "<strong>Taxe : </strong>" . Cart::instance('shopping')->tax() . " &euro;",
            'subtotal' => "<strong>Sous-total :</strong>" . Cart::instance('shopping')->subtotal() . "&euro",
            'product' => $addCart
        ];
    ...
    public function editProduct(Request $request)
    {
        ...
        $total = $shippingPrice + $total;
        return view($this->viewPath . 'cart', compact('total','shippingPrice','coupon'));
    }


    /**
     * Ajoute un code promo au panier
     * @param Request $request
    ...
    public function coupon(Request $request)
    {
        ...
        $messages = [
            "success" => "Votre code promo a bien été appliqué",
            "noCoupon" => "Aucun code promo n'existe.",
            "noAccess" => "Vous n'avez accès à ce code promo.",
            "noConnect" => "Vous devez être connecté pour utiliser ce code promo",
            "noProduct" => "Le code promo ne concerne pas vos produits."
        ];
    ...
    /**
     * Supprimer le code promo en session
     * @return \Illuminate\Http\RedirectResponse
     */
    ...
    /**
     * Création d'un compte ou d'un guest
    ...
    /**
     * Affiche la liste des adresses + création d'une adresse
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    ...
    /**
     * Sauvegarde des adresses en session et en BDD si utilisateur connecté
     * @param \Modules\ShoppingCart\Http\Requests\UsersAdresses $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function orderAddressStore(\Modules\ShoppingCart\Http\Requests\UsersAdresses $request)
    {
        $user = session()->get('custommers');
        $billing = 0;
    ...
    public function orderShipping()
    {
        ...
        if(empty($address) || empty($address['delivery']) || empty($address['billing'])) {
            session()->flash('error',"Veuillez selectionner vos adresses");
            return redirect()->route('cart.user.address');
        }
    ...
    public function orderShippingStore(SelectCarrier $request)
    {
        ...
        if(is_null($carrier)) {
            session()->flash('error',"Le transporteur sélectionné n'existe pas.");
            return redirect()->route('cart.user.shipping');
        }
        $total = Cart::instance('shopping')->total(2,".","");
        $carrierPrice = null;
        ...
        if(is_null($carrierPrice)){
            session()->flash('error',"Le transporteur sélectionné ne correspond pas à votre panier.");
            return redirect()->route('cart.user.shipping');
        }
    ...
    public function orderRecap()
    {
        ...
        if(empty($shipping) || empty($shipping['id'])) {
            session()->flash('error',"Veuillez selectionner un transporteur");
            return redirect()->route('cart.user.shipping');
        }
        $total = Cart::instance('shopping')->total(2,".","");
        $coupon = [];
    ...
    public function process()
    {
        ...
        /* A SUPPRIMER */
        $address['delivery']['gender'] = 'M';
        $address['billing']['gender'] = 'M';
    ...
    public function orderReturn(Request $request)
    {
        ...
        $refOrder = config('ikCommerce.orderRef') .
            str_pad($nbOrder, config('ikCommerce.refCount'), 0, STR_PAD_LEFT);
        ...
        /* Génération du PDF */
        $invoice = new \App\Http\Picl0u\Invoice($order);
    ...
    public function orderTest(Request $request)
    {
        ...
        //mail('vincent@ikon-k.fr', "Infos paiement", print_r($payment, true));
        $id = "1T0GXxPoV3EmycuKZ2291522160995844";
        $token = "1T0GXxPoV3EmycuKZ2291522160995844";
        ...
        foreach(Cart::instance('shopping')->content() as $key => $row){
            ...
            $productsOrder[$key] = [
                ...
                'price_ht' => $row->price,
                'price_ttc' => $row->total,
                'declinaisons' => $row->options->declinaison
            ];
        ...
        /* Génération du PDF */
        $invoice = new \App\Http\Picl0u\Invoice($order);
        if(!empty(setting('generals.invoiceNote')) && !is_null(setting('generals.invoiceNote'))) {
            $invoice->notes(setting('generals.invoiceNote'));
        }
        if(!empty(setting('generals.invoiceFooter')) && !is_null(setting('generals.invoiceFooter'))) {
            $invoice->footer(setting('generals.invoiceFooter'));
        }
        $invoiceLink = $invoice->generate();
    ...
    private function adressesUser($user)
    {
        ...
        if(!is_null($user->id) && !empty($user->id)){
            $addressList = $user->UsersAdresses->toArray();
        } else {
            $addressList = session()->get('custommersAddresses');
        }
        if (empty($addressList)) {
