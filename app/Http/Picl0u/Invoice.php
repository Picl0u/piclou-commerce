...
class Invoice{
    ...
    public function __construct(Order $order, string $name = null)
    {
        $this->order = $order;
        $this->name = $name;
        $this->layout = config('ikCommerce.invoiceLayout');
    }
    ...
    public function generate(string $path = null)
    {
        ...
        if(is_null($path)){
            $path = config('ikCommerce.invoicePath');
        }
        if(is_null($this->name)){
            $this->name = config('ikCommerce.invoiceName')."-".$order->reference;
        }
        ...
        $pdf->render();
        $fileName = config('ikCommerce.invoicePath') . "/" . $this->name . ".pdf";
    ...
    private function merchantInfos()
    {
        return [
            ...
            'tva'       => setting("generals.invoiceTVA"),
            'rcs'       => setting("generals.invoiceRCS"),
